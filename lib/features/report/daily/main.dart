...
class _Main_State extends State<Main_> {
  ...
  //
  void _fallback(String date_str) {
    raw_transactions = _sample_data();
    // demo values
    check_ins = 14;
    check_outs = 10;
    total_guests_today = 18;
    active_stays = 32;
    total_rooms = 40;
    occupied_rooms = 32;
    available_rooms = 8;
    ...
    for (final tx in raw_transactions) {
      final amt = (tx[schema.AMOUNT] as num?)?.toDouble() ?? 0.0;
      final pay = tx[schema.PAYMENT_METHOD].toString();
      if (tx[schema.CATEGORY] == "Room Charge") {
        room_paid_total += amt;
        if (pay.contains("ABA")) room_bank_usd += amt;
        if (pay.contains("Cash")) room_cash_usd += amt;
      } else {
        revenue_paid_total += amt;
        if (pay.contains("ABA")) revenue_bank_usd += amt;
        if (pay.contains("Cash")) revenue_cash_usd += amt;
      }
    }
    total_revenue = room_paid_total + revenue_paid_total;
    aba_bank_total = room_bank_usd + revenue_bank_usd;
    cash_total = room_cash_usd + revenue_cash_usd;
  }

  //
  void _parse(Map<String, dynamic> res, String date_str) {
    // summary
    final s = res["summary"] as Map<String, dynamic>? ?? {};
    check_ins = (s["total_check_ins"] as num?)?.toInt() ?? 0;
    check_outs = (s["total_check_outs"] as num?)?.toInt() ?? 0;
    total_guests_today = (s["total_guests_today"] as num?)?.toInt() ?? 0;
    active_stays = (s["active_stays"] as num?)?.toInt() ?? 0;

    // room_revenue
    final rr = res["room_revenue"] as Map<String, dynamic>? ?? {};
    room_paid_total = (rr["paid_total_usd"] as num?)?.toDouble() ?? 0;
    room_bank_usd = (rr["paid_bank_usd"] as num?)?.toDouble() ?? 0;
    room_cash_usd = (rr["paid_cash_usd"] as num?)?.toDouble() ?? 0;
    room_balance = (rr["balance_total_usd"] as num?)?.toDouble() ?? 0;

    // revenue (service charges)
    final rv = res["revenue"] as Map<String, dynamic>? ?? {};
    revenue_paid_total = (rv["paid_total_usd"] as num?)?.toDouble() ?? 0;
    revenue_bank_usd = (rv["paid_bank_usd"] as num?)?.toDouble() ?? 0;
    revenue_cash_usd = (rv["paid_cash_usd"] as num?)?.toDouble() ?? 0;
    revenue_balance = (rv["balance_total_usd"] as num?)?.toDouble() ?? 0;
    ...
    // room_status
    final st = res["room_status"] as Map<String, dynamic>? ?? {};
    total_rooms = (st["total_rooms"] as num?)?.toInt() ?? 0;
    occupied_rooms = (st["occupied"] as num?)?.toInt() ?? 0;
    available_rooms = (st["available"] as num?)?.toInt() ?? 0;
    pending_pay = (st["pending_pay"] as num?)?.toInt() ?? 0;
    pending_leave = (st["pending_leave"] as num?)?.toInt() ?? 0;
    pending_clean = (st["pending_clean"] as num?)?.toInt() ?? 0;

    // transactions list
    final txs = res["transactions"];
    raw_transactions = (txs is List && txs.isNotEmpty) ? List<Map<String, dynamic>>.from(txs) : _sample_data();
  }
  ...
  List<Map<String, dynamic>> _sample_data() => [
    {
      schema.ID: "1",
      schema.TIME: "08:15 AM",
      schema.REFERENCE_NO: "INV-001",
      schema.ROOM_NUMBER: "102",
      schema.GUEST_NAME: "Sok Dara",
      schema.CATEGORY: "Room Charge",
      schema.PAYMENT_METHOD: "ABA Pay",
      schema.AMOUNT: 120.00,
      schema.STATUS: "Completed",
      schema.STAFF: "Vannak",
    },
    {
      schema.ID: "2",
      schema.TIME: "09:30 AM",
      schema.REFERENCE_NO: "INV-002",
      schema.ROOM_NUMBER: "205",
      schema.GUEST_NAME: "John Smith",
      schema.CATEGORY: "Room Charge",
      schema.PAYMENT_METHOD: "Credit Card",
      schema.AMOUNT: 180.00,
      schema.STATUS: "Completed",
      schema.STAFF: "Vannak",
    },
    {
      schema.ID: "3",
      schema.TIME: "10:05 AM",
      schema.REFERENCE_NO: "INV-003",
      schema.ROOM_NUMBER: "104",
      schema.GUEST_NAME: "Chan Thy",
      schema.CATEGORY: "Minibar",
      schema.PAYMENT_METHOD: "Cash",
      schema.AMOUNT: 25.50,
      schema.STATUS: "Completed",
      schema.STAFF: "Bora",
    },
    {
      schema.ID: "4",
      schema.TIME: "01:45 PM",
      schema.REFERENCE_NO: "INV-004",
      schema.ROOM_NUMBER: "301",
      schema.GUEST_NAME: "Keo Sophea",
      schema.CATEGORY: "Laundry",
      schema.PAYMENT_METHOD: "ABA Pay",
      schema.AMOUNT: 35.00,
      schema.STATUS: "Completed",
      schema.STAFF: "Bora",
    ...
    {
      schema.ID: "5",
      schema.TIME: "03:20 PM",
      schema.REFERENCE_NO: "INV-005",
      schema.ROOM_NUMBER: "201",
      schema.GUEST_NAME: "Michael Brown",
      schema.CATEGORY: "Room Charge",
      schema.PAYMENT_METHOD: "Credit Card",
      schema.AMOUNT: 250.00,
      schema.STATUS: "Completed",
      schema.STAFF: "Sreypov",
    },
  ];
