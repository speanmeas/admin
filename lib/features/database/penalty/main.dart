...
class _Main_State extends State<Main_> {
  ...
  Future<void> on_rows_moved(PlutoGridOnRowsMovedEvent e) async {
    ...
    final String id = moved_row.cells[Penalty.ID]?.value?.toString() ?? "";
    if (id.isEmpty) return;

    final String? prev_id = idx > 0 ? state_manager.rows[idx - 1].cells[Penalty.ID]?.value?.toString() : null;
    final String? next_id = idx < state_manager.rows.length - 1 ? state_manager.rows[idx + 1].cells[Penalty.ID]?.value?.toString() : null;
    ...
    final res = await dio.post(endpoint.PENALTY_UPDATE_ORDER, data: body);
  ...
  Future<void> on_reload() async {
    final tmp = await dio.post(endpoint.PENALTY_READ_COUNT);
  ...
  Future<void> on_load_page(int p) async {
    ...
    final data = List<Penalty>.from((tmp.data ?? const []).map((d) => Penalty.fromJson(d)));
          ...
          cells: {
            for (var c in list_column_pluto) //
              c.field: (() {
                ...
                if (c.field == Penalty.ID) return PlutoCell(value: d.id ?? "");
                if (c.field == Penalty.NAME) return PlutoCell(value: d.name ?? "");
                if (c.field == Penalty.PRICE) return PlutoCell(value: d.price ?? 0.0);
                if (c.field == Penalty.NOTE) return PlutoCell(value: d.note ?? "");
    ...
    final row = PlutoRow(
      cells: {
        for (var c in list_column_pluto) //
          c.field: PlutoCell(
            value: (() {
              if (c.field == "action") return "";
              if (c.field == Penalty.ID) return "";
  ...
  Future<void> do_create(PlutoRow row) async {
    ...
    if (created_id != null) {
      row.cells[Penalty.ID]!.value = created_id;
