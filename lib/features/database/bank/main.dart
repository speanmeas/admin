...
class _Main_State extends State<Main_> {
  ...
  Future<void> on_rows_moved(PlutoGridOnRowsMovedEvent e) async {
    ...
    final String id = moved_row.cells[Bank.ID]?.value?.toString() ?? "";
    if (id.isEmpty) return;

    final String? prev_id = idx > 0 ? state_manager.rows[idx - 1].cells[Bank.ID]?.value?.toString() : null;
    final String? next_id = idx < state_manager.rows.length - 1 ? state_manager.rows[idx + 1].cells[Bank.ID]?.value?.toString() : null;
    ...
    final res = await dio.post(endpoint.BANK_UPDATE_ORDER, data: body);
  ...
  Future<void> on_load_page(int p) async {
    ...
    final data = List<Bank>.from((tmp.data ?? const []).map((d) => Bank.fromJson(d)));
        ...
        PlutoRow(
          cells: {
            for (var c in list_column_pluto) //
              c.field: (() {
                if (c.field == Bank.ID) return PlutoCell(value: d.id ?? "");
                if (c.field == "index") return PlutoCell(value: 0);
                if (c.field == "action") return PlutoCell(value: "");
                if (c.field == Bank.NAME) return PlutoCell(value: d.name ?? "");
                if (c.field == Bank.NOTE) return PlutoCell(value: d.note ?? "");
  ...
  Future<void> do_create(PlutoRow row) async {
    ...
    if (created_id != null) {
      row.cells[Bank.ID]!.value = created_id;
  ...
  Future<void> do_updated(String? id, String field, dynamic value) async {
    final tmp = await dio.post(endpoint.BANK_UPDATE, data: {Bank.ID: id, field: value});
    if (tmp == null) {
      await on_reload();
      snackbar(ct: context, ms: dio.error_msg ?? "", cl: Colors.red);
    }
