...
class _Main_State extends State<Main_> {
  ...
  Future<void> on_rows_moved(PlutoGridOnRowsMovedEvent e) async {
    ...
    final String id = state_manager.rows[idx].cells[Demo_2_2.ID]?.value?.toString() ?? "";
    if (id.isEmpty) return;

    final String? prev_id = idx > 0 ? state_manager.rows[idx - 1].cells[Demo_2_2.ID]?.value?.toString() : null;
    final String? next_id = idx < state_manager.rows.length - 1 ? state_manager.rows[idx + 1].cells[Demo_2_2.ID]?.value?.toString() : null;

    final res = await dio.post(
      endpoint.DEMO_2_2_UPDATE_ORDER,
      data: {
        "_id": id,
  ...
  Future<void> on_reload() async {
    final tmp = await dio.post(endpoint.DEMO_2_2_READ_COUNT);
  ...
  Future<void> on_load_page(int p) async {
    ...
    final data = List<Demo_2_2>.from((tmp.data ?? const []).map((d) => Demo_2_2.fromJson(d)));
          ...
          cells: {
            for (var c in list_column_pluto) //
              c.field: (() {
                ...
                if (c.field == Demo_2_2.ID) return PlutoCell(value: d.id ?? "");
                if (c.field == "index") return PlutoCell(value: "");
                if (c.field == Demo_2_2.TEXT) return PlutoCell(value: d.text ?? "");
                if (c.field == Demo_2_2.NUMBER) return PlutoCell(value: d.number ?? 0);
  ...
  Future<void> do_create(PlutoRow row) async {
    final tmp = await dio.post(endpoint.DEMO_2_2_CREATE);
    if (tmp == null) {
      await on_reload();
    ...
    if (created_id != null) {
      row.cells[Demo_2_2.ID]!.value = created_id;
  ...
  void on_delete(PlutoColumnRendererContext rc) {
    ...
    final id = rc.row.cells[Demo_2_2.ID]?.value;
    if (id == null) return;
    do_delete(id);
  }

  Future<void> do_delete(String? id) async {
    final tmp = await dio.post(endpoint.DEMO_2_2_DELETE, data: {Demo_2_2.ID: id});
    if (tmp == null) {
      await on_reload();
      snackbar(ct: context, ms: dio.error_msg ?? "", cl: Colors.red);
      return;
  ...
  void on_updated(PlutoGridOnChangedEvent e) {
    final id = e.row.cells[Demo_2_2.ID]?.value;
    do_updated(id, e.column.field, e.value);
  }

  Future<void> do_updated(String? id, String field, dynamic value) async {
    final tmp = await dio.post(endpoint.DEMO_2_2_UPDATE, data: {Demo_2_2.ID: id, field: value});
    if (tmp == null) {
      await on_reload();
      snackbar(ct: context, ms: dio.error_msg ?? "", cl: Colors.red);
    }
...
// ថ្នាក់ Main_ ជាទំព័រគ្រប់គ្រង Demo_2_2
