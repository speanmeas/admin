...
class _Main_State extends State<Main_> {
        ...
        PlutoRow(
          cells: {
            for (var c in list_column) //
              c: (() {
                if (c == Demo_2_2.ID) return PlutoCell(value: d.id ?? "");
                if (c == "index") return PlutoCell(value: i + 1);
                if (c == "action") return PlutoCell(value: "");
                if (c == Demo_2_2.TEXT) return PlutoCell(value: d.text ?? "");
                if (c == Demo_2_2.NUMBER) return PlutoCell(value: d.number ?? 0);
  ...
  void on_changed(PlutoGridOnChangedEvent e) async {
    ...
    final tmp = await dio.post(endpoint.DEMO_2_2_UPDATE, data: {Demo_2_2.ID: id, e.column.field: e.value});
...
// * ថ្នាក់ Main_ ជាទំព័រគ្រប់គ្រងឧទាហរណ៍ (Demo 2-2)
