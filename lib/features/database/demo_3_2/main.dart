...
class _Main_State extends State<Main_> {
  ...
  void on_load_page(int p) async {
    ...
    if (tmp == null) return snackbar(ct: context, ms: dio.error_msg ?? "", cl: Colors.red);
    data = List<Demo_3_2>.from((tmp.data ?? const []).map((d) => Demo_3_2.fromJson(d)));
        ...
        PlutoRow(
          cells: {
            for (var c in list_column) //
              c: (() {
                if (c == Demo_3_2.ID) return PlutoCell(value: d.id ?? "");
                if (c == "index") return PlutoCell(value: i + 1);
                if (c == "action") return PlutoCell(value: "");
                if (c == Demo_3_2.TEXT) return PlutoCell(value: d.text ?? "");
                if (c == Demo_3_2.NUMBER) return PlutoCell(value: d.number ?? 0);
  ...
  void on_changed(PlutoGridOnChangedEvent e) async {
    ...
    final tmp = await dio.post(endpoint.DEMO_3_2_UPDATE, data: {Demo_3_2.ID: id, e.column.field: e.value});
