// * ទំព័រគ្រប់គ្រងmini bar សម្រាប់បង្កើត អាន កែ និងលុប
...
// * បង្កើត layout មេរបស់ទំព័រគ្រប់គ្រងmini bar
...
class _Main_State extends State<Main_> {
  ...
  void init() async {
    ...
    tmp = await dio.post(endpoint.MINI_BAR_CRUD_READ_COUNT, data: {"count": true});
    setState(() => is_loading = false);
    if (tmp == null) return snackbar(ct: context, ms: "Error: ${endpoint.MINI_BAR_CRUD_READ_COUNT}", cl: Colors.red);
  ...
  void load_page(int p) async {
    // * អានទិន្នន័យmini barតាម offset និង limit
    setState(() => is_loading = true);
    tmp = await dio.post(
    ...
    if (tmp == null) return snackbar(ct: context, ms: "Error: ${endpoint.MINI_BAR_CRUD_READ}", cl: Colors.red);
...
// * ថ្នាក់ Main_ ជាទំព័រគ្រប់គ្រងmini bar
