...
class _Main_State extends State<Main_> {
  ...
  Future<void> on_rows_moved(PlutoGridOnRowsMovedEvent e) async {
    ...
    final String id = moved_row.cells[Mini_Bar.ID]?.value?.toString() ?? "";
    if (id.isEmpty) return;

    final String? prev_id = idx > 0 ? state_manager.rows[idx - 1].cells[Mini_Bar.ID]?.value?.toString() : null;
    final String? next_id = idx < state_manager.rows.length - 1 ? state_manager.rows[idx + 1].cells[Mini_Bar.ID]?.value?.toString() : null;
    ...
    final res = await dio.post(endpoint.MINI_BAR_UPDATE_ORDER, data: body);
  ...
  Future<void> on_reload() async {
    final tmp = await dio.post(endpoint.MINI_BAR_READ_COUNT);
