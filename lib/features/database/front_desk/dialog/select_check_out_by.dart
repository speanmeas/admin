...
// * បង្ហាញ dialog ជ្រើសរើសអ្នកបញ្ចប់ (check_out_by) ពីបញ្ជីអ្នកប្រើ
// * — ប្រើតែ CRUD endpoints ប៉ុណ្ណោះ (USER_READ + FRONT_DESK_UPDATE)
Future<bool?> dialog_select_check_out_by({
                    ...
                    : () async {
                        ...
                        dynamic tmp_fd = await dio.post(
                          ...
                          data: {
                            ...
                            Front_Desk.CHECK_OUT_BY: new_user_id, //
                          },
                        );
                        if (tmp_fd == null) {
    ...
    return Scaffold(
      body: Center(
        child: OutlinedButton(
          ...
          onPressed: () async {
            final v = await dialog_select_check_out_by(context: context, front_desk_id: "test");
            if (v == null) return;
            tmp = v;
            setState(() {});
          },
