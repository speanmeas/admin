...
class _Main_State extends State<Main_> {
  ...
  // ########## BLOCK METHODS ##########
  void on_loaded(PlutoGridOnLoadedEvent e) {
  ...
  Future<void> on_rows_moved(PlutoGridOnRowsMovedEvent e) async {
    ...
    final String id = moved_row.cells[Nationality.ID]?.value?.toString() ?? "";
    if (id.isEmpty) return;

    final String? prev_id = idx > 0 ? state_manager.rows[idx - 1].cells[Nationality.ID]?.value?.toString() : null;
    final String? next_id = idx < state_manager.rows.length - 1 ? state_manager.rows[idx + 1].cells[Nationality.ID]?.value?.toString() : null;
    ...
    final res = await dio.post(endpoint.NATIONALITY_UPDATE_ORDER, data: body);
  ...
  Future<void> on_load_page(int p) async {
    ...
    final data = List<Nationality>.from((tmp.data ?? const []).map((d) => Nationality.fromJson(d)));
          ...
          cells: {
            for (var c in list_column_pluto) //
              c.field: (() {
                ...
                if (c.field == Nationality.ID) return PlutoCell(value: d.id ?? "");
                if (c.field == Nationality.NAME) return PlutoCell(value: d.name ?? "");
                if (c.field == Nationality.NOTE) return PlutoCell(value: d.note ?? "");
  ...
  Future<void> do_create(PlutoRow row) async {
    ...
    if (created_id != null) {
      row.cells[Nationality.ID]!.value = created_id;
  ...
  void on_updated(PlutoGridOnChangedEvent e) {
    final id = e.row.cells[Nationality.ID]?.value;
    do_updated(id, e.column.field, e.value);
  }

  Future<void> do_updated(String? id, String field, dynamic value) async {
    final tmp = await dio.post(endpoint.NATIONALITY_UPDATE, data: {Nationality.ID: id, field: value});
    if (tmp == null) {
      await on_reload();
      snackbar(ct: context, ms: dio.error_msg ?? "", cl: Colors.red);
    }
...
// ថ្នាក់ Main_ ជាទំព័រគ្រប់គ្រងសញ្ជាតិ
