...
class _Main_State extends State<Main_> {
        ...
        columns: [
          ...
          PlutoColumn(
            ...
            renderer: (rc) {
              ...
              final options = demos.map((d) => d.text ?? "").where((s) => s.isNotEmpty).toList();
              return Center(
                child: SizedBox(
                  width: 140,
                  height: 28,
  ...
  Future<void> on_rows_moved(PlutoGridOnRowsMovedEvent e) async {
    ...
    final String id = state_manager.rows[idx].cells[Demo_2_1.ID]?.value?.toString() ?? "";
    if (id.isEmpty) return;

    final String? prev_id = idx > 0 ? state_manager.rows[idx - 1].cells[Demo_2_1.ID]?.value?.toString() : null;
    final String? next_id = idx < state_manager.rows.length - 1 ? state_manager.rows[idx + 1].cells[Demo_2_1.ID]?.value?.toString() : null;

    final res = await dio.post(
      endpoint.DEMO_2_1_UPDATE_ORDER,
      data: {
        "_id": id,
  ...
  Future<void> on_reload() async {
    final tmp = await dio.post(endpoint.DEMO_2_1_READ_COUNT);
    ...
    final td = await dio.post(endpoint.DEMO_2_2_READ, data: {"key": DEFAULT_KEY, "order": DEFAULT_ORDER, "offset": 0, "limit": 200});
    if (td != null) demos = List<Demo_2_2>.from((td.data ?? const []).map((d) => Demo_2_2.fromJson(d)));
  ...
  Future<void> on_load_page(int p) async {
    ...
    if (tmp == null) return snackbar(ct: context, ms: dio.error_msg ?? "", cl: Colors.red);
    data = List<Demo_2_1>.from((tmp.data ?? const []).map((d) => Demo_2_1.fromJson(d)));
          ...
          cells: {
            for (var c in list_column_pluto) //
              c.field: (() {
                ...
                if (c.field == Demo_2_1.ID) return PlutoCell(value: d.id ?? "");
                if (c.field == Demo_2_1.TEXT) return PlutoCell(value: d.text ?? "");
                if (c.field == Demo_2_1.NUMBER) return PlutoCell(value: d.number ?? 0);
                if (c.field == Demo_2_1.DEMO_2_2_ID) {
                  final dd = d.demo_2_2_id;
                  return PlutoCell(value: dd is Demo_2_2 ? (dd.text ?? "") : (dd is Map ? (dd["text"]?.toString() ?? "") : ""));
                }
  ...
  Future<void> do_create(PlutoRow row) async {
    final tmp = await dio.post(endpoint.DEMO_2_1_CREATE);
    if (tmp == null) {
      await on_reload();
    ...
    if (created_id != null) {
      row.cells[Demo_2_1.ID]!.value = created_id;
  ...
  void on_updated(PlutoGridOnChangedEvent e) {
    final id = e.row.cells[Demo_2_1.ID]?.value;
    do_updated(id, e.column.field, e.value);
  }

  void on_change_demo(PlutoColumnRendererContext rc, String id, String name) {
    final row_id = rc.row.cells[Demo_2_1.ID]?.value;
    if (row_id == null) return;
    rc.cell.value = name;
    state_manager.notifyListeners();
    do_updated(row_id, Demo_2_1.DEMO_2_2_ID, id);
  }

  Future<void> do_updated(String? id, String field, dynamic value) async {
    final tmp = await dio.post(endpoint.DEMO_2_1_UPDATE, data: {Demo_2_1.ID: id, field: value});
    if (tmp == null) {
      await on_reload();
      snackbar(ct: context, ms: dio.error_msg ?? "", cl: Colors.red);
    }
...
// ថ្នាក់ Main_ ជាទំព័រគ្រប់គ្រងឧទាហរណ៍ (Demo 2-1)
