// * ទំព័រគ្រប់គ្រងឧទាហរណ៍ (Demo 2-1) សម្រាប់បង្កើត អាន កែ និងលុប
...
class _Main_State extends State<Main_> {
  ...
  void init() async {
    ...
    tmp = await dio.post(endpoint.DEMO_2_1_READ_COUNT, data: {"count": true});
    setState(() => is_loading = false);
    if (tmp == null) return snackbar(ct: context, ms: "Error: ${endpoint.DEMO_2_1_READ_COUNT}", cl: Colors.red);
  ...
  void load_page(int p) async {
    ...
    if (tmp == null) return snackbar(ct: context, ms: "Error: ${endpoint.DEMO_2_1_READ}", cl: Colors.red);
    ...
    // * បម្លែងទិន្នន័យទៅជា List<Demo_2_1> ដើម្បីបង្កើត PlutoRow
          ...
          cells: {
            for (var c in list_c) //
              c: (() {
                ...
                if (c == Demo_2_1.NUMBER) //
                  return PlutoCell(value: demo.number);
                if (c == Demo_2_1.DEMO_2_2_ID) //
                  return PlutoCell(value: demo.demo_2_2_id?.text);
  ...
  void on_read() async {
    ...
    final id = row?.cells[Demo_2_1.ID]?.value?.toString() ?? "";
