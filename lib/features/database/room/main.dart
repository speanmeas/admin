...
class _Main_State extends State<Main_> {
  ...
  void init() async {
    ...
    tmp = await dio.post(endpoint.ROOM_READ_COUNT, data: {"count": true});
    setState(() => is_loading = false);
    if (tmp == null) return snackbar(ct: context, ms: "Error: ${endpoint.ROOM_READ_COUNT}", cl: Colors.red);
          ...
          cells: {
            for (var c in list_c) //
              c: (() {
                ...
                if (c == Room.PRICE_PER_DAY) //
                  return PlutoCell(value: Room.PRICE_PER_DAY);
                if (c == Room.PRICE_PER_3H) //
                  return PlutoCell(value: Room.PRICE_PER_3H);
                if (c == Room.KIND) //
