...
class _Main_State extends State<Main_> {
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    (() {
                      ...
                      double usd_per_3h =
                          (r[sm_room.USD_PER_3H] as num?)?.toDouble() ?? 0;
                      double usd_per_day =
                          (r[sm_room.USD_PER_DAY] as num?)?.toDouble() ?? 0;
                      return Row(
                        spacing: 4,
                        mainAxisAlignment: MainAxisAlignment.center,
                        children: [
                          ...
                          Text("-"), //
                          Text(
                            "${usd_per_3h.toStringAsFixed(2)} \$ / 3 ${t("Hours")}",
                            style: TextStyle(
                              fontSize: 14,
                              fontWeight: FontWeight.bold,
                            ),
                          ...
                          Text("-"), //
                          Text(
                            "${usd_per_day.toStringAsFixed(2)} \$ / 1 ${t("Days")}",
                            style: TextStyle(
                              fontSize: 14,
                              fontWeight: FontWeight.bold,
                            ),
                    ...
                    if (r[sm_room.FRONT_DESK_ID] != null) ...[
                      ...
                      if (!["Pending Fix"].contains(r[sm_room.STATUS]))
                        (() {
                          tmp = _fd(r);
                          final stay_n_guest =
                              tmp[sm_front_desk.CHECK_IN_NUMBER] ?? "0";
                          final stay_day =
                              tmp[sm_front_desk.CHECK_IN_DAY] ?? "0";
                          final stay_hour =
                              tmp[sm_front_desk.CHECK_IN_HOUR] ?? "0";
                          return Row(
                            spacing: 4,
                            children: [
                      ...
                      if (!["Pending Fix"].contains(r[sm_room.STATUS]))
                        (() {
                          ...
                          if (tmp.isNotEmpty)
                            price = double.parse(
                              tmp.last["pay_price"]?.toString() ?? "0",
                            );
                          for (var l in tmp) {
                            pay =
                                pay +
                                double.parse(l["pay_cash"]?.toString() ?? "0");
                            pay =
                                pay +
                                double.parse(l["pay_bank"]?.toString() ?? "0");
                            change =
                                change +
                                double.parse(
                                  l["pay_return"]?.toString() ?? "0",
                                );
                          }
                          return Row(
                            spacing: 4,
                      ...
                      if (r[sm_room.STATUS] != "Pending Fix")
                        (() {
                          ...
                          if (tmp[sm_front_desk.CHECK_IN_DUE] != null) {
                            final dt = DateTime.parse(
                              tmp[sm_front_desk.CHECK_IN_DUE],
                            );
                      ...
                      if (r[sm_room.STATUS] != "Pending Fix")
                        (() {
                          ...
                          if (tmp[sm_front_desk.CHECK_OUT_AT] != null) {
                            final dt = DateTime.parse(
                              tmp[sm_front_desk.CHECK_OUT_AT],
                            );
                      ...
                      if (r[sm_room.STATUS] == "Pending Fix")
                        (() {
                          ...
                          if (tmp[sm_front_desk.BROKE_AT] != null) {
                            final dt = DateTime.parse(
                              tmp[sm_front_desk.BROKE_AT],
                            );
                            broke_date = DateFormat(
                              DEFAULT_DATE_FORMAT,
                            ).format(dt);
                          }
                          return Row(
  ...
  void on_check_in(dynamic r) async {
    try {
      tmp = await Navigator.push(
        context,
        MaterialPageRoute(
          builder: (context) => check_in.Main_(room_id: r[sm_room.ID]),
        ),
      );
      if (tmp != null) init();
    } catch (e, st) {
  ...
  void on_payment(dynamic r) async {
    try {
      tmp = await Navigator.push(
        context,
        MaterialPageRoute(
          builder: (context) => pay_room.Main_(room_id: r[sm_room.ID]),
        ),
      );
      if (tmp != null) init();
    } catch (e, st) {
  ...
  void on_check_out(dynamic r) async {
    try {
      tmp = await Navigator.push(
        context,
        MaterialPageRoute(
          builder: (context) => check_out.Main_(room_id: r[sm_room.ID]),
        ),
      );
      if (tmp != null) init();
    } catch (e, st) {
  ...
  void on_clean(dynamic r) async {
    try {
      tmp = await Navigator.push(
        context,
        MaterialPageRoute(
          builder: (context) => clean.Main_(room_id: r[sm_room.ID]),
        ),
      );
      if (tmp != null) init();
    } catch (e, st) {
  ...
  void on_broke(dynamic r) async {
    try {
      tmp = await Navigator.push(
        context,
        MaterialPageRoute(
          builder: (context) => broke.Main_(room_id: r[sm_room.ID]),
        ),
      );
      if (tmp != null) init();
    } catch (e, st) {
  ...
  void on_fix(dynamic r) async {
    try {
      tmp = await Navigator.push(
        context,
        MaterialPageRoute(
          builder: (context) => fix.Main_(room_id: r[sm_room.ID]),
        ),
      );
      if (tmp != null) init();
    } catch (e, st) {
  ...
  void on_detail(dynamic r) async {
    try {
      tmp = await Navigator.push(
        context,
        MaterialPageRoute(
          builder: (context) => detail.Main_(room_id: r[sm_room.ID]),
        ),
      );
      if (tmp != null) init();
    } catch (e, st) {
  ...
  void on_cancel(dynamic r) async {
    try {
      tmp = await Navigator.push(
        context,
        MaterialPageRoute(
          builder: (context) => cancel.Main_(room_id: r[sm_room.ID]),
        ),
      );
      if (tmp != null) init();
    } catch (e, st) {
  ...
  void on_change_room(dynamic r) async {
    try {
      tmp = await Navigator.push(
        context,
        MaterialPageRoute(
          builder: (context) => change_room.Main_(room_id: r[sm_room.ID]),
        ),
      );
      if (tmp != null) init();
    } catch (e, st) {
  ...
  void on_update_stay(dynamic r) async {
    try {
      tmp = await Navigator.push(
        context,
        MaterialPageRoute(
          builder: (context) => update_stay.Main_(room_id: r[sm_room.ID]),
        ),
      );
      if (tmp != null) init();
    } catch (e, st) {
  ...
  void on_update_guest(dynamic r) async {
    try {
      tmp = await Navigator.push(
        context,
        MaterialPageRoute(
          builder: (context) => update_guest.Main_(room_id: r[sm_room.ID]),
        ),
      );
      if (tmp != null) init();
    } catch (e, st) {
  ...
  void on_update_room_payment(dynamic r) async {
    try {
      tmp = await Navigator.push(
        context,
        MaterialPageRoute(
          builder: (context) => update_pay_room.Main_(room_id: r[sm_room.ID]),
        ),
      );
      if (tmp != null) init();
    } catch (e, st) {
  ...
  void on_pay_other(dynamic r) async {
    try {
      tmp = await Navigator.push(
        context,
        MaterialPageRoute(
          builder: (context) => pay_other.Main_(room_id: r[sm_room.ID]),
        ),
      );
      if (tmp != null) init();
    } catch (e, st) {
  ...
  Map<String, dynamic> _fd(dynamic r) =>
      map_fd[r[sm_room.ID]] as Map<String, dynamic>? ?? {};
